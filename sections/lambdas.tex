...
clbks.push_back([](float v) { return std::log(v); });
...
\begin{frame}[fragile]{Lambdas and callbacks}{\texttt{std::function} and \texttt{std::bind}}
	\begin{columns}
		\column{.9\textwidth}
		\begin{lstlisting}[style=codebeamer, language=C++, caption=\texttt{std::bind} usage example to bind a function to some of its arguments.]
#include <functional>
#include <iostream>

int add(int a, int b) { return a + b; }

// Bind first argument to 10, second remains free
// A ``placeholder'' (special value set later on call) is used
